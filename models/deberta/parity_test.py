#!/usr/bin/env python3
"""
DeBERTa v1 Parity Test — Hidden State Comparison.

Loads microsoft/deberta-base with both HuggingFace and our vLLM encoder,
then compares the hidden state outputs to verify numerical parity.

Usage:
    cd /workspace/vllm-factory
    python models/deberta/parity_test.py
"""
...
TEST_TEXTS = [
    ...
    "DeBERTa uses disentangled attention with relative position bias.",
    "This is a test sentence for verifying parity between HuggingFace and vLLM implementations.",
]


def load_hf_model():
    """Load the reference HuggingFace DeBERTa model."""
...
def load_vllm_model(hf_model):
    ...
    vllm_model = DebertaEncoderModel(config=config)
    vllm_model.eval()
