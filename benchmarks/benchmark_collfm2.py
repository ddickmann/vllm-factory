#!/usr/bin/env python3
"""
Benchmark: ColLFM2 — vLLM in-process (text-only, no vanilla reference)

Measures req/s for multimodal LFM2-VL ColPali retrieval at ~512 tokens with batch=1,8,32.
"""
import gc
import os
import time
...
def bench_vllm():
    import plugins.collfm2  # noqa: register plugin
...
if __name__ == "__main__":
    print("=" * 70)
    print(f"  ColLFM2 Benchmark — vLLM ({MODEL})")
    print("=" * 70)

    r = bench_vllm()
    print(f"\n{'=' * 70}")
    print(f"  RESULTS — ColLFM2 (vLLM only, no vanilla reference)")
    print(f"{'=' * 70}")
    for bs in [1, 8, 32]:
