...
for idx, wid in enumerate(word_ids_list):
    if wid is not None and wid != prev_wid:
        ...
        prev_wid = wid
print(f"Text tokenized: {len(words)} words", flush=True)

# Pre-encode both label sets
...
print(f"Generic embs: {generic_embs.shape}, Entity embs: {entity_embs.shape}", flush=True)
...
ents_generic = run_and_decode(generic_labels, generic_embs, threshold=0.3)
for e in sorted(ents_generic, key=lambda x: -x["score"]):
    print(f"  '{e['text']}' -> {e['label']} (score={e['score']:.4f})")
if not ents_generic:
    print("  (none found)")

print("\n=== ENTITY-SPECIFIC LABELS ===", flush=True)
ents_entity = run_and_decode(entity_labels, entity_embs, threshold=0.3)
for e in sorted(ents_entity, key=lambda x: -x["score"]):
    print(f"  '{e['text']}' -> {e['label']} (score={e['score']:.4f})")
if not ents_entity:
    print("  (none found)")
