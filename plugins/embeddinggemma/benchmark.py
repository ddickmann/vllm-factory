"""
EmbeddingGemma Throughput Benchmark

Measures req/s, latency (p50/p95/p99) for the EmbeddingGemma plugin.
Supports --seq-len to control input token length for realistic benchmarks.

Usage:
    python plugins/embeddinggemma/benchmark.py \
        --model <model-id> \
        --num-requests 1000 \
        --concurrency 32 \
        --seq-len 128
...
def main():
    p = argparse.ArgumentParser(description="EmbeddingGemma throughput benchmark")
    p.add_argument("--model", default="unsloth/embeddinggemma-300m")
