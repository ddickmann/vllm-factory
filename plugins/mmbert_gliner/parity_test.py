"""
ModernBERT GLiNER Parity Test — VAGOsolutions/SauerkrautLM-GLiNER

Step 1: Get reference output from GLiNER library
Step 2: Prepare local model dir with proper config.json for vLLM
Step 3: Get vLLM output using mmbert_gliner plugin
Step 4: Compare decoded entities and benchmark

Mirrors mt5_gliner/parity_test.py exactly for comparable results.

Usage:
    pip install gliner
    VLLM_PLUGINS=mmbert_gliner python plugins/mmbert_gliner/parity_test.py
"""
...
# Force spawn for vLLM subprocess when CUDA is already initialized
# (GLiNER reference runs on GPU before vLLM loads)
os.environ["VLLM_WORKER_MULTIPROC_METHOD"] = "spawn"

import torch

MODEL = "VAGOsolutions/SauerkrautLM-GLiNER"
LOCAL_MODEL_DIR = "/tmp/sauerkraut-gliner-vllm"
...
def prepare_local_model(gliner_model):
    ...
    # ModernBERT-based config for vLLM
    real_num_layers = encoder_config.get("num_hidden_layers", 22)
    vllm_config = {
        "model_type": "gliner_mmbert",
        "architectures": ["GLiNERModernBertModel"],
        # vLLM pooling: num_hidden_layers=0 -> no KV cache allocation
        # The real layer count is stored in encoder_num_layers for the model
        "num_hidden_layers": 0,
        "num_attention_heads": 1,
        "encoder_num_layers": real_num_layers,
        "encoder_num_attention_heads": encoder_config.get("num_attention_heads", 12),
        "hidden_size": encoder_config.get("hidden_size", 768),
        "intermediate_size": encoder_config.get("intermediate_size", 2048),
        "vocab_size": encoder_config.get("vocab_size", 256002),
        "max_position_embeddings": encoder_config.get("max_position_embeddings", 8192),
        "hidden_activation": encoder_config.get("hidden_activation", "gelu"),
        "norm_eps": encoder_config.get("norm_eps", encoder_config.get("layer_norm_eps", 1e-5)),
        "pad_token_id": encoder_config.get("pad_token_id", 0),
        ...
        "local_rope_theta": encoder_config.get("local_rope_theta", 10000.0),
        # GLiNER params
        ...
        "max_len": gliner_config.get("max_len", 2048),
    }
...
def run_vllm_gliner():
    """Get predictions from vLLM mmbert_gliner plugin.
    ...
    print("\n" + "=" * 60)
    print("STEP 3: vLLM mmbert_gliner Plugin")
    print("=" * 60)
