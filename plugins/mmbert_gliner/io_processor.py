"""
IOProcessor plugin for mmbert_gliner — server-side GLiNER NER via vLLM's
native IOProcessor pipeline.

Replaces the Forge BaseProcessor + pooling patch approach with vLLM's built-in
IOProcessor ABC. The serving layer handles request parsing, pre/post-processing,
and response shaping entirely inside vLLM.

Entry-point group: vllm.io_processor_plugins
Entry-point name:  mmbert_gliner_io
...
class MMBertGLiNERIOProcessor(IOProcessor[GLiNERInput, list[dict[str, Any]]]):
    """IOProcessor for mmbert_gliner — GLiNER NER with ModernBERT backbone.
    ...
    def __init__(self, vllm_config: VllmConfig):
        ...
        # Stashed between pre_process and validate_or_generate_params (same coroutine)
        self._pending_extra_kwargs: dict | None = None
        # Keyed by request_id for post_process correlation
...
def get_processor_cls() -> str:
    ...
    return "plugins.mmbert_gliner.io_processor.MMBertGLiNERIOProcessor"
